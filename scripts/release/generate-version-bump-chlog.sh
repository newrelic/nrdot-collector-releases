...
#
# Usage: generate-version-bump-chlog.sh -c <current_beta_core> -n <next_beta_core> -r <current_beta_fork> -f <next_beta_fork>
#   -c  current otel beta core version (e.g. v0.147.0)
#   -n  next otel beta core version    (e.g. v0.148.0)
#   -r  current otel beta fork version (e.g. v0.147.0-nr1)
...
while getopts c:n:r:f: flag
do
    case "${flag}" in
        c) CURRENT_BETA_CORE=${OPTARG};;
        n) NEXT_BETA_CORE=${OPTARG};;
        r) CURRENT_BETA_FORK=${OPTARG};;
        f) NEXT_BETA_FORK=${OPTARG};;
        *) exit 1;;
    esac
done

if [[ -z "$CURRENT_BETA_CORE" || -z "$NEXT_BETA_CORE" || -z "$CURRENT_BETA_FORK" || -z "$NEXT_BETA_FORK" ]]; then
    echo "Usage: $0 -c <current_beta_core> -n <next_beta_core> -r <current_beta_fork> -f <next_beta_fork>" >&2
    exit 1
fi
...
if [[ "$CURRENT_BETA_CORE" != "$NEXT_BETA_CORE" ]]; then
    ...
    echo "otel core version unchanged (${CURRENT_BETA_CORE}), skipping core changelog entry"
fi
...
if [[ "$CURRENT_BETA_FORK" != "$NEXT_BETA_FORK" ]]; then
    ...
    echo "nr-fork version unchanged (${CURRENT_BETA_FORK}), skipping fork changelog entry"
fi
