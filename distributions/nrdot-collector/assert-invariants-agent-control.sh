...
core_distro_dir="${script_dir}"

# assert config file location
test -f "${core_distro_dir}/config.yaml" ||
...
for env_var in "${env_vars[@]}"; do
  ...
  grep -E '\${env:[^}]+}' "${core_distro_dir}/config.yaml" |
  grep "${env_var}" -q ||
...
for goreleaser_yaml in "${goreleaser_yamls[@]}"; do
  ...
  yq -e '.builds[].binary == "nrdot-collector"' "${core_distro_dir}/${goreleaser_yaml}" ||
    { echo "expected binary name 'nrdot-collector' in ${goreleaser_yaml}"; exit 1; }
  yq -e '.nfpms[].package_name == "nrdot-collector"' "${core_distro_dir}/${goreleaser_yaml}" ||
